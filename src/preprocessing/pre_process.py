...
from named_entity_recognition.database_value_finder.database_value_finder_sparql import \
    DatabaseValueFinderSparql
...
def build_kg_value_finder(db_name, schema_path, args):

    return DatabaseValueFinderSparql(db_name, schema_path)
...
def main():
    ...
    """[(idx, query) for idx, query in enumerate(data) if query['question'] == "Which are the car makers which produce at least 2 models and more than 3 car makers ? List the id and the maker ."]
    data = data[171:172]
    ner_data = ner_data[171:172]
    ...
    results = [pre_process(idx, example, ner_information, build_kg_value_finder(example['db_id'], args.table_path, args), is_training=True)
