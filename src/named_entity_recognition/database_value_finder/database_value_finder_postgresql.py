...
class DatabaseValueFinderPostgreSQL(DatabaseValueFinder):
    ...
    def find_similar_values_in_database(self, potential_values, include_primary_keys):
        ...
        table_text_column_mapping = self._get_relevant_columns(
            include_primary_keys, column_types=['text'])

        # For numbers, we do an exact matching. We also don't include PKs/FKs, as we almost always get a match with those
        table_number_column_mapping = self._get_relevant_columns(
            False, column_types=['number'])

        # Same for characters
        table_character_column_mapping = self._get_relevant_columns(
            False, column_types=['character'])
        ...
        try:
            ...
            #
            potential_character_values = _filter_character_values(
                potential_values)
    ...
    def _find_matches_in_column_using_similarity(self, table, column, potential_values, connection):
        ...
        try:
            ...
            print("Error found during value finding...")
    ...
    def _find_matches_in_column_by_exact_matching(self, table, column, potential_values, connection):
        ...
        query=f"""
            ...
            """
        try: 
            cursor.execute(query, potential_values)
            rows = cursor.fetchall()
        except Exception as e:
            print("Error found during value finding...")
            print(e.__traceback__, "\n", e)
